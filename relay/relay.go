...
type RelayManager struct {
	mu        sync.RWMutex
	engines   map[string]*core.Engine        // project name → engine (runtime only)
	bindings  map[string]*core.RelayBinding  // chatID → binding
	storePath string                         // empty = no persistence
	timeout   time.Duration
}

// Ensure RelayManager satisfies the core interface at compile time.
var _ core.RelayManagerAPI = (*RelayManager)(nil)

func NewRelayManager(dataDir string) *RelayManager {
	rm := &RelayManager{
		engines:  make(map[string]*core.Engine),
		bindings: make(map[string]*core.RelayBinding),
...
func (rm *RelayManager) RegisterEngine(name string, e *core.Engine) {
	rm.mu.Lock()
	defer rm.mu.Unlock()
	rm.engines[name] = e
}
...
func (rm *RelayManager) Bind(platform, chatID string, bots map[string]string) {
	rm.mu.Lock()
	defer rm.mu.Unlock()
	rm.bindings[chatID] = &core.RelayBinding{
		Platform: platform,
		ChatID:   chatID,
		Bots:     bots,
	}
...
func (rm *RelayManager) GetBinding(chatID string) *core.RelayBinding {
	rm.mu.RLock()
	defer rm.mu.RUnlock()
...
func (rm *RelayManager) Send(ctx context.Context, req core.RelayRequest) (*core.RelayResponse, error) {
	...
	response, err := targetEngine.HandleRelayRequest(relayCtx, req.From, chatID, req.Message)
	if err != nil {
	...
	return &core.RelayResponse{Response: response}, nil
}

// sendToGroup sends a message to the group chat for visibility.
func (rm *RelayManager) sendToGroup(ctx context.Context, e *core.Engine, platform, sessionKey, content string) {
...
func (rm *RelayManager) saveLocked() {
	...
	if err := core.AtomicWriteFile(rm.storePath, data, 0o644); err != nil {
...
func (rm *RelayManager) load() {
	...
	var bindings map[string]*core.RelayBinding
