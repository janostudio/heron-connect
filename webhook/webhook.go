...
func (ws *WebhookServer) RegisterEngine(name string, e *core.Engine) {
	ws.mu.Lock()
	defer ws.mu.Unlock()
	ws.engines[name] = e
}
...
func (ws *WebhookServer) resolveEngine(project string) (*core.Engine, error) {
	ws.mu.RLock()
	defer ws.mu.RUnlock()
...
func (ws *WebhookServer) executePrompt(engine *core.Engine, sessionKey, prompt string, silent bool, event string) {
	platformName := ""
	if idx := strings.Index(sessionKey, ":"); idx > 0 {
		platformName = sessionKey[:idx]
	}

	var targetPlatform core.Platform
	...
	rc, ok := targetPlatform.(core.ReplyContextReconstructor)
	if !ok {
	...
	session := engine.GetSessions().GetOrCreateActive(sessionKey)
	if !session.TryLock() {
...
func (ws *WebhookServer) executeShell(engine *core.Engine, req WebhookRequest, event string) {
	...
	workDir := req.WorkDir
	if workDir == "" {
		if wd, ok := engine.GetAgent().(interface{ GetWorkDir() string }); ok {
