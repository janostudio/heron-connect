...
func TestBuildWindowsTaskScript(t *testing.T) {
	cfg := Config{
		BinaryPath: `C:\Program Files\heron-connect\heron-connect.exe`,
		WorkDir:    `C:\Users\me\.heron-connect`,
		LogFile:    `C:\Users\me\.heron-connect\logs\heron-connect.log`,
	...
	for _, want := range []string{
		...
		`Set-Location -LiteralPath 'C:\Users\me\.heron-connect'`,
		`while ($true) {`,
		`& 'C:\Program Files\heron-connect\heron-connect.exe'`,
...
func TestWindowsTaskActionRunsHidden(t *testing.T) {
	got := windowsTaskAction(`C:\Users\me\.heron-connect\heron-connect-daemon.ps1`)
	for _, want := range []string{
		`powershell.exe`,
		`-WindowStyle Hidden`,
		`-NoProfile`,
		...
		`-File "C:\Users\me\.heron-connect\heron-connect-daemon.ps1"`,
	} {
...
func TestWindowsTaskCreateUsesLimitedInteractivePrincipal(t *testing.T) {
	...
	if err := createWindowsTask(`C:\Users\me\.heron-connect\heron-connect-daemon.ps1`); err != nil {
	...
	for _, want := range []string{
		...
		`-RunLevel Limited`,
		`-WindowStyle Hidden`,
		`C:\Users\me\.heron-connect\heron-connect-daemon.ps1`,
	} {
		if !strings.Contains(script, want) {
...
func TestWindowsTaskMatchesActionRequiresExactAction(t *testing.T) {
	...
	if !windowsTaskMatchesAction(`C:\Users\me\.heron-connect\heron-connect-daemon.ps1`) {
		t.Fatal("windowsTaskMatchesAction() = false, want true")
	}
	for _, want := range []string{
		`$expectedArgs = '-WindowStyle Hidden -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "C:\Users\me\.heron-connect\heron-connect-daemon.ps1"'`,
...
func TestPowerShellLiteralEscapesSingleQuotes(t *testing.T) {
	got := powerShellLiteral(`C:\Users\O'Brien\.heron-connect`)
	want := `'C:\Users\O''Brien\.heron-connect'`
	if got != want {
