...
func TestBuildWindowsTaskScript(t *testing.T) {
	cfg := Config{
		...
		WorkDir:    `C:\Users\me\.cc-connect-qhn`,
		LogFile:    `C:\Users\me\.cc-connect-qhn\logs\cc-connect.log`,
	...
	for _, want := range []string{
		`$env:CC_LOG_FILE = 'C:\Users\me\.cc-connect-qhn\logs\cc-connect.log'`,
		`$env:CC_LOG_MAX_SIZE = '10485760'`,
		`$env:PATH = 'C:\Program Files\nodejs;C:\Users\me\AppData\Local\Programs'`,
		`$env:HTTPS_PROXY = 'http://127.0.0.1:7890'`,
		`$env:http_proxy = 'http://127.0.0.1:7890'`,
		`Set-Location -LiteralPath 'C:\Users\me\.cc-connect-qhn'`,
		`while ($true) {`,
...
func TestWindowsTaskActionRunsHidden(t *testing.T) {
	got := windowsTaskAction(`C:\Users\me\.cc-connect-qhn\cc-connect-daemon.ps1`)
	for _, want := range []string{
		`powershell.exe`,
		`-WindowStyle Hidden`,
		`-NoProfile`,
		...
		`-File "C:\Users\me\.cc-connect-qhn\cc-connect-daemon.ps1"`,
	} {
...
func TestWindowsTaskCreateUsesLimitedInteractivePrincipal(t *testing.T) {
	...
	if err := createWindowsTask(`C:\Users\me\.cc-connect-qhn\cc-connect-daemon.ps1`); err != nil {
	...
	for _, want := range []string{
		...
		`-RunLevel Limited`,
		`-WindowStyle Hidden`,
		`C:\Users\me\.cc-connect-qhn\cc-connect-daemon.ps1`,
	} {
		if !strings.Contains(script, want) {
...
func TestWindowsTaskMatchesActionRequiresExactAction(t *testing.T) {
	...
	if !windowsTaskMatchesAction(`C:\Users\me\.cc-connect-qhn\cc-connect-daemon.ps1`) {
		t.Fatal("windowsTaskMatchesAction() = false, want true")
	}
	for _, want := range []string{
		`$expectedArgs = '-WindowStyle Hidden -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "C:\Users\me\.cc-connect-qhn\cc-connect-daemon.ps1"'`,
...
func TestPowerShellLiteralEscapesSingleQuotes(t *testing.T) {
	got := powerShellLiteral(`C:\Users\O'Brien\.cc-connect-qhn`)
	want := `'C:\Users\O''Brien\.cc-connect-qhn'`
	if got != want {
