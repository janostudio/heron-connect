...
func (m *systemdManager) buildUnit(cfg Config) string {
	...
	sb.WriteString("[Unit]\n")
	sb.WriteString("Description=cc-connect-qhn - AI Agent Chat Bridge\n")
...
func checkSystemdRunning(system bool) error {
	...
	if system {
		if wsl {
			return fmt.Errorf("systemd is not active in this WSL2 instance.\n" +
				...
				"    [boot]\n" +
				"    systemd=true\n" +
				"  Or use: nohup cc-connect-qhn > cc-connect-qhn.log 2>&1 &")
		}
		if state == "offline" || strings.Contains(state, "not been booted") {
			return fmt.Errorf("systemd is not active (state: %s).\n"+
				...
				"    nohup cc-connect-qhn > cc-connect-qhn.log 2>&1 &", state)
		}
		return fmt.Errorf("systemd check failed (state: %s).\n"+
			"  Use nohup as alternative: nohup cc-connect-qhn > cc-connect-qhn.log 2>&1 &", state)
	}
	...
	return fmt.Errorf("systemd user session not available.\n"+
		...
		"  Try one of:\n"+
		"    1. Run as root: sudo cc-connect-qhn daemon install (uses system-level systemd)\n"+
		"    2. loginctl enable-linger %s && export XDG_RUNTIME_DIR=/run/user/$(id -u)\n"+
		"    3. Use nohup/tmux instead: nohup cc-connect-qhn > cc-connect-qhn.log 2>&1 &", user)
}
