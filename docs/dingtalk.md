...
本文档介绍如何将 **heron-connect** 接入钉钉，让你可以通过钉钉机器人远程调用 Claude Code。
...
> ⚠️ **重要**：请妥善保存这两个凭证，后续配置 heron-connect 时需要用到。AppSecret 只会显示一次。
...
启动后，heron-connect 会自动与钉钉建立 Stream 长连接。你会在日志中看到：
