...
本文档介绍如何将 **cc-connect-qhn** 接入飞书，让你可以通过飞书机器人远程调用 Claude Code。

## 前置要求

- 飞书账号（个人或企业均可）
- 一台可运行 cc-connect-qhn 的设备（无需公网 IP）
...
如果你已经装好 `cc-connect-qhn`，可以直接用内置命令完成“新建机器人/关联已有机器人”，并自动写回 `config.toml`：
...
> ⚠️ **重要**：请妥善保存这两个凭证，后续配置 cc-connect-qhn 时需要用到。App Secret 只会显示一次，如果忘记了需要重置。

### 2.3 配置到 cc-connect-qhn

将凭证配置到 cc-connect-qhn 的 `config.toml` 中：
...
启动后，cc-connect-qhn 会自动与飞书建立 WebSocket 长连接。你会在日志中看到：
...
cc-connect-qhn: 🤔 思考中...
cc-connect-qhn: 🔧 执行: Bash(ls -la)
cc-connect-qhn: ✅ 这是一个 Node.js 项目，包含以下目录...
```
...
│                      你的本地环境                            │
│                                                              │
│   cc-connect-qhn ◄──► Claude Code CLI ◄──► 你的项目代码         │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
cc-connect-qhn 内置了自动重连机制，断开后会自动尝试重新连接。

### Q: 消息发送后没有响应？

检查以下项目：
1. cc-connect-qhn 服务是否正常运行
2. 长连接是否建立成功（查看日志）
3. 事件订阅是否配置了 `im.message.receive_v1`

### Q: 点击卡片按钮没有反应或报错？

cc-connect-qhn 默认使用交互卡片显示权限确认、provider 选择等操作。如果点击按钮后无响应、显示加载超时或报错，请检查：
...
通常是飞书注册模板侧的展示文案，不影响返回 `app_id/app_secret` 和接入 cc-connect-qhn。
