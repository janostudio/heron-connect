...
本文档介绍如何将 **heron-connect** 接入微博私信，让你可以通过微博私信远程调用 AI 编程 Agent。
...
> ⚠️ **重要**：请妥善保存这两个凭证，后续配置 heron-connect 时需要用到。

---

## 第二步：配置 heron-connect
...
启动后，heron-connect 会自动与微博建立 WebSocket 长连接。你会在日志中看到：
...
heron-connect: ✅ 这是一个 Go 项目，包含以下模块...
```

### 4.2 使用命令

所有 heron-connect 命令均可在微博私信中使用：
...
│                      你的本地环境                            │
│                                                              │
│   heron-connect ◄──► AI Agent CLI ◄──► 你的项目代码            │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
微博私信文本限制约 2000 字符。heron-connect 会自动将超长消息分块发送，接收端会按顺序收到完整内容。
...
heron-connect 内置了自动重连机制（指数退避，最大 10 秒间隔），断开后会自动尝试重新连接。
...
微博私信有约 2000 字符的限制，heron-connect 会自动分块发送。如果仍有问题，检查网络连接。
