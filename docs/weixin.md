...
本文档说明如何通过 **heron-connect** 接入**微信个人号**侧的对话能力。底层使用腾讯 **ilink 机器人 HTTP 网关**（与 OpenClaw 插件 `openclaw-weixin` 同类接口：`getUpdates` 长轮询 + `sendMessage` 下发）。
...
- 可运行 heron-connect 的环境（无需公网 IP；ilink 由云端提供）
...
装好 `heron-connect` 后，在项目目录执行（将 `my-project` 换成你的 `config.toml` 里的项目名，或留空在仅有一个项目时自动选择）：
...
```bash
heron-connect weixin bind --project my-project --token '<你的_Bearer_Token>'
# 或
heron-connect weixin setup --project my-project --token '<你的_Bearer_Token>'
```
...
网关下发消息时可能带 `context_token`；heron-connect 会缓存并在回复时使用。  
**首次连接**：请先启动 heron-connect，再用允许的微信账号 **给机器人发一条消息**，完成关联后再使用 `/new` 等指令。
...
- **出站图片与文件**：平台实现了 `ImageSender` / `FileSender`，可通过 `heron-connect send --image` / `--file` 等能力下发（需引擎侧已支持附件发送）。  
...
```bash
go build -tags no_weixin ./cmd/heron-connect
```
