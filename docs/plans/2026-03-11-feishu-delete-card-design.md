...
1. User sends `/delete`.
2. If the platform supports cards, heron-connect renders a delete-mode session list card.
...
6. The confirmation card provides:
   - `确认删除`
   - `返回继续选择`
7. On confirmation, heron-connect deletes the selected sessions, skips the active session, clears the temporary selection state, and renders a result card.
