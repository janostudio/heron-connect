...
本文档介绍如何将 **heron-connect** 接入企业微信，让你可以通过企业微信（甚至个人微信）远程调用 Claude Code。
...
企业微信「智能机器人」支持 WebSocket 长连接模式，heron-connect 主动连接企业微信服务器，无需公网 URL、无需消息加解密、无需 IP 白名单，配置最简单。

### 前置要求

- 企业微信管理员权限
- 一台可运行 heron-connect 的服务器（**无需公网 IP**）
...
> ⚠️ **暂时不要点保存！** 需要先启动 heron-connect 再回来保存（因为保存时企业微信会立即验证回调 URL）。
...
这样 heron-connect 调用企业微信 API 时会通过 VPS 代理，出口 IP 固定为 VPS 的 IP。
...
│                    你的服务器                                  │
│                                                              │
│   cloudflared ──→ heron-connect ──→ Claude Code CLI             │
│   / ngrok            │                                       │
│                      │ (可选) proxy                          │
│                      ▼                                       │
│                企业微信 API ──→ VPS 正向代理                   │
...
1. 确认 heron-connect 已启动且 webhook server 在监听
