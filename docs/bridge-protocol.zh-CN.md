...
Bridge 协议允许使用**任何编程语言**编写的外部平台适配器在运行时通过 WebSocket 动态接入 heron-connect，无需编写 Go 代码或重新编译二进制文件。
...
`BridgePlatform` 是 heron-connect 内置的一个平台实现，它：
...
| `reply_ctx` | string | 是 | 不透明的上下文字符串，适配器需要它来路由回复。heron-connect 会在每个回复中原样回传。 |
...
发送结构化卡片给用户。仅在适配器声明了 `"card"` 能力时发送；否则 heron-connect 会降级为 `reply`，内容使用 `card.RenderText()` 生成的纯文本。
...
3. 恢复正常运行 — heron-connect 独立于连接维护会话状态。

### 消息顺序

单个 WebSocket 连接内的消息是有序的。heron-connect 按 session key 顺序处理适配器消息。

### 超时

- **Ping 间隔**：适配器应至少每 30 秒发送一次 `ping`。
- **连接超时**：heron-connect 在 90 秒没有收到 ping 后关闭空闲连接。
- **回复超时**：如果 agent 耗时过长，heron-connect 可能发送错误回复。适配器不需要特殊处理。
...
1. **保持无状态** — 适配器应该是一个轻量的协议转换层。所有会话状态存储在 heron-connect 中。
...
协议版本通过 `register` 消息的 `metadata.protocol_version` 声明。当前版本为 `1`。heron-connect 会拒绝不兼容版本的连接，并在 `register_ack` 中返回错误。
