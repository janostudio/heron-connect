...
```
   ...
   user → MAX cloud ─── HTTPS POST ───▶  │  nginx :443 (TLS)                     │
                       ...
                       /webhook           │                                       │
                                          │  heron-connect (HTTP :8090, localhost)   │
                                          └───────────────────────────────────────┘
```
...
```
                                          ┌─── VPS ───┐         ┌──── Home ────┐
   user → MAX cloud ─── HTTPS ─────────▶ │  nginx    │ ──SSH──▶│  heron-connect  │
...
3. `systemctl restart heron-connect` (or however you run it).
...
1. `systemctl --user status heron-connect` — is it running?
