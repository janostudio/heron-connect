...
```
   ...
   user → MAX cloud ─── HTTPS POST ───▶  │  nginx :443 (TLS)                     │
                       ...
                       /webhook           │                                       │
                                          │  cc-connect-qhn (HTTP :8090, localhost)   │
                                          └───────────────────────────────────────┘
```
...
```
                                          ┌─── VPS ───┐         ┌──── Home ────┐
   user → MAX cloud ─── HTTPS ─────────▶ │  nginx    │ ──SSH──▶│  cc-connect-qhn  │
...
3. `systemctl restart cc-connect-qhn` (or however you run it).
...
1. `systemctl --user status cc-connect-qhn` — is it running?
