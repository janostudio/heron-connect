...
The Bridge Protocol allows **external platform adapters** written in any programming language to connect to heron-connect at runtime via WebSocket. This eliminates the requirement to write Go code and recompile the binary for every new platform integration.
...
The `BridgePlatform` is a built-in platform inside heron-connect that:
...
| `reply_ctx` | string | yes | Opaque context string the adapter needs to route replies back. heron-connect echoes this in every reply. |
...
Keepalive. heron-connect responds with `pong`.
...
Send a structured card to the user. Only sent if the adapter declared `"card"` capability; otherwise heron-connect falls back to `reply` with `card.RenderText()`.
...
If a capability is not declared, heron-connect will automatically degrade:
...
3. Resume normal operation — heron-connect maintains session state independently of the connection.

### Message Ordering

Messages within a single WebSocket connection are ordered. heron-connect processes adapter messages sequentially per session key.

### Timeouts

- **Ping interval**: Adapters should send `ping` at least every 30 seconds.
- **Connection timeout**: heron-connect closes idle connections after 90 seconds without a ping.
- **Reply timeout**: If an agent takes too long, heron-connect may send an error reply. The adapter does not need to handle this specially.
...
1. **Keep it stateless** — the adapter should be a thin translation layer. All session state lives in heron-connect.
...
The protocol version is declared in the `register` message via `metadata.protocol_version`. The current version is `1`. heron-connect will reject connections with incompatible versions and respond with a `register_ack` containing an error.
