...
heron-connect 通过 [QQ 官方机器人 API v2](https://bot.q.qq.com/wiki/) 连接 QQ，无需第三方适配器，无需公网 IP。

heron-connect connects to QQ via the [official QQ Bot Platform API v2](https://bot.q.qq.com/wiki/). No third-party adapter needed, no public IP required.
...
```
QQ Open Platform ←WebSocket→ heron-connect ←→ Agent (Claude Code / etc.)
```
...
### 2. 配置 heron-connect / Configure heron-connect
...
**Q: 断线重连？/ Reconnection?**
- heron-connect 内置自动重连机制，断线后会自动尝试恢复（最多 30 次）
- heron-connect has built-in automatic reconnection with resume support (up to 30 attempts)
