...
cc-connect-qhn 通过 [QQ 官方机器人 API v2](https://bot.q.qq.com/wiki/) 连接 QQ，无需第三方适配器，无需公网 IP。

cc-connect-qhn connects to QQ via the [official QQ Bot Platform API v2](https://bot.q.qq.com/wiki/). No third-party adapter needed, no public IP required.
...
```
QQ Open Platform ←WebSocket→ cc-connect-qhn ←→ Agent (Claude Code / etc.)
```
...
### 2. 配置 cc-connect-qhn / Configure cc-connect-qhn
...
**Q: 断线重连？/ Reconnection?**
- cc-connect-qhn 内置自动重连机制，断线后会自动尝试恢复（最多 30 次）
- cc-connect-qhn has built-in automatic reconnection with resume support (up to 30 attempts)
