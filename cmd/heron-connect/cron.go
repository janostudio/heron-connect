...
func runCronAdd(args []string) {
	...
	if _, err := os.Stat(sockPath); os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "Error: heron-connect is not running (socket not found: %s)\n", sockPath)
		os.Exit(1)
	}
...
func runCronInfo(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Usage: heron-connect cron info <id> [field]")
		fmt.Fprintln(os.Stderr, "Use 'heron-connect cron list' to see all task IDs.")
		os.Exit(1)
	}
...
func runCronEdit(args []string) {
	...
	if id == "" {
		fmt.Fprintln(os.Stderr, "Error: job ID is required")
		fmt.Fprintln(os.Stderr, "Run 'heron-connect cron edit --help' for usage.")
		os.Exit(1)
	}
	if field == "" {
...
func printCronUsage() {
	fmt.Println(`Usage: heron-connect cron <command> [options]
...
Run 'heron-connect cron <command> --help' for details.`)
}

func printCronAddUsage() {
	fmt.Println(`Usage: heron-connect cron add [options] [<min> <hour> <day> <month> <weekday> <prompt>]
...
Examples:
  heron-connect cron add --cron "0 6 * * *" --prompt "Collect GitHub trending data" --desc "Daily Trending"
  heron-connect cron add --cron "*/30 * * * *" --exec "df -h" --desc "Disk usage check"
  heron-connect cron add 0 6 * * * Collect GitHub trending data and send me a summary`)
}

func printCronEditUsage() {
	fmt.Println(`Usage: heron-connect cron edit <id> <field> <value> [options]
...
Examples:
  ...
  heron-connect cron edit abc123 description "Daily standup reminder"
  heron-connect cron edit abc123 timeout_mins 60
  heron-connect cron edit abc123 mute true`)
}
