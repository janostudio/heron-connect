...
const (
	githubRepo   = "janostudio/heron-connect"
	...
	giteeAPI     = "https://gitee.com/api/v5/repos/janostudio/heron-connect/releases/latest"
)
...
func getUpdateHintIfAvailable() string {
	...
	if isNewer(cachedVer, version) {
		return fmt.Sprintf("\n📦 Update available: %s → %s  (run: heron-connect update)\n", version, cachedVer)
	}
	return ""
}
...
func runUpdate() {
	...
	fmt.Println("Restart heron-connect to use the new version.")
}
...
func binaryAssetName(tag string) string {
	...
	name := fmt.Sprintf("heron-connect-%s-%s-%s", tag, goos, goarch)
	if goos == "windows" {
		name += ".exe"
	}
	return name
...
func archiveAssetName(tag string) string {
	goos := runtime.GOOS
	goarch := runtime.GOARCH
	base := fmt.Sprintf("heron-connect-%s-%s-%s", tag, goos, goarch)
	if goos == "windows" {
		return base + ".zip"
	}
	return base + ".tar.gz"
...
// extractBinaryFromArchive extracts the heron-connect binary from a .tar.gz or .zip archive.
...
func extractFromTarGz(archivePath string) (string, error) {
	...
	for {
		...
		if strings.HasPrefix(hdr.Name, "heron-connect") {
			tmp, err := os.CreateTemp("", "heron-connect-update-*")
			if err != nil {
				return "", err
			}
...
func extractFromZip(archivePath string) (string, error) {
	...
	for _, f := range r.File {
		if !strings.HasPrefix(f.Name, "heron-connect") {
			continue
		}
		rc, err := f.Open()
		if err != nil {
...
func checkUpdate() {
	...
	if isNewer(release.TagName, version) {
		hint := "heron-connect update"
		if release.Prerelease {
			hint = "heron-connect update --pre"
		}
...
// (node_modules/heron-connect/bin/) and updates the package.json version to
...
func syncNpmPackageVersion(execPath, newVer string) {
	...
	if !strings.Contains(name, "heron-connect") {
		return
	}
	...
	if err := os.WriteFile(pkgJSON, out, 0o644); err != nil {
		...
		fmt.Println("   please run: npm update -g heron-connect")
	} else {
