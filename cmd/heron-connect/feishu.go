...
func runFeishuSetup(args []string, requestedMode string) {
	...
	case feishuSetupModeNew:
		...
		if err != nil {
			...
			fmt.Fprintln(os.Stderr, "Tip: you can bind an existing bot with `heron-connect feishu bind --app app_id:app_secret`")
			os.Exit(1)
		}
...
func printFeishuUsage() {
	fmt.Println(`Usage: heron-connect feishu <command> [options]
...
Examples:
  ...
  heron-connect feishu new --project my-project --platform-type lark`)
}
...
func resolveFeishuSetupInputs(mode, app, appID, appSecret string) (effectiveMode, resolvedAppID, resolvedAppSecret string, err error) {
	...
	if mode == feishuSetupModeNew && (appID != "" || appSecret != "") {
		return "", "", "", fmt.Errorf("new mode does not accept credentials; use `heron-connect feishu bind`")
	}
