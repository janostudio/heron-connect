...
func TestSyncNpmPackageVersion_NormalizesVPrefix(t *testing.T) {
	...
	dir := t.TempDir()
	heronConnectDir := filepath.Join(dir, "node_modules", "heron-connect")
	binDir := filepath.Join(heronConnectDir, "bin")
	if err := os.MkdirAll(binDir, 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	execPath := filepath.Join(binDir, "heron-connect")

	pkgJSON := filepath.Join(heronConnectDir, "package.json")
	pkgData := `{"name": "heron-connect", "version": "v1.0.0"}`
...
func TestSyncNpmPackageVersion_UpdatesWhenDifferent(t *testing.T) {
	...
	pkgJSON := filepath.Join(heronConnectDir, "package.json")
	pkgData := `{"name": "heron-connect", "version": "v0.9.0"}`
	if err := os.WriteFile(pkgJSON, []byte(pkgData), 0o644); err != nil {
		t.Fatalf("write pkg.json: %v", err)
	}
