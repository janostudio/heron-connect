...
// heron-connect instances with the same config from running simultaneously.
...
func AcquireInstanceLock(configPath string) (*InstanceLock, error) {
	...
	if err != nil {
		...
		if pid > 0 {
			return nil, fmt.Errorf("another heron-connect instance is already running (PID %d) with config %s", pid, configPath)
		}
		return nil, fmt.Errorf("another heron-connect instance is already running with config %s", configPath)
	}
