...
// runDoctor dispatches `heron-connect doctor ...`. Today the only subcommand
// is `user-isolation`, but this function is the growth point for future
// diagnostics.
func runDoctor(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: heron-connect doctor <subcommand>")
...
func runDoctorUserIsolation(args []string) {
	...
	outPath := fs.String("out", "", "path to write JSON report (default: ~/.heron-connect/audits/<timestamp>-<project>.json per project)")
...
// defaultAuditDir returns ~/.heron-connect/audits for the supervisor user.
func defaultAuditDir() (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".heron-connect", "audits"), nil
}
