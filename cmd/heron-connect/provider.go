...
func printProviderUsage() {
	fmt.Println(`Usage: heron-connect provider <command> [options]
...
Examples:
  ...
  heron-connect provider add --project my-backend --name bedrock --env CLAUDE_CODE_USE_BEDROCK=1,AWS_PROFILE=bedrock
  heron-connect provider list --project my-backend
  heron-connect provider remove --project my-backend --name relay
  heron-connect provider import --project my-backend
  heron-connect provider presets
  heron-connect provider global list
  heron-connect provider global add --name minimaxi --api-key sk-xxx --base-url https://api.minimaxi.chat/v1`)
}
...
func runProviderGlobal(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, `Usage: heron-connect provider global <command>
...
func runGlobalProviderList(args []string) {
	...
	if len(providers) == 0 {
		...
		fmt.Println("\nAdd one with: heron-connect provider global add --name <name> --api-key <key>")
		return
	}
