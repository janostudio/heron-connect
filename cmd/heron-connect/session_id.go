...
func printAgentSIDUsage() {
	fmt.Println(`Usage: heron-connect agent-sid [options]

Print the agent session ID (e.g. Claude Code, Codex, Gemini CLI) for the
current session. This is the ID used for --resume.

The command reads from the persisted session file; no running heron-connect
...
Examples:
  heron-connect agent-sid                         Auto-detect from env (inside a session)
  heron-connect agent-sid -p mybot -s "discord:123:456"`)
}
