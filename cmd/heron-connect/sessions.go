...
func runSessions(args []string) {
	...
	case "show":
		...
		if id == "" {
			...
			fmt.Fprintln(os.Stderr, "Usage: heron-connect sessions show <session-id> [-n N]")
			os.Exit(1)
		}
...
func runSessionsShow(dataDir, id string, limit int) {
	...
	if record == nil {
		...
		fmt.Fprintln(os.Stderr, "Use 'heron-connect sessions list' to see available sessions.")
		os.Exit(1)
	}
...
func printSessionsUsage() {
	fmt.Println(`Usage: heron-connect sessions [command] [options]
...
Examples:
  heron-connect sessions                           Interactive TUI browser
  heron-connect sessions list                      List all sessions
  heron-connect sessions show "mybot:s1"           Show all messages in session
  heron-connect sessions show "#1" -n 20           Show last 20 messages of first session`)
}
