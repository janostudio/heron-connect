...
- **Web 端代码块复制恢复可用**。此前代码块右上角的复制按钮「点了没反应」——按钮正常显示、点击也有成功反馈，但剪贴板里是空的。根因是取待复制文本时写死了一个假设：`<code>` 元素的子节点必须是「裸字符串」。而代码块一旦被高亮（`rehype-highlight` 上色），子节点就变成了 `<span class="hljs-*">` 与文本片段交织的**数组**，判据不成立 → 待复制文本落到空串 → 复制了个寂寞。因此这个 bug 与语言无关，只要代码块被成功高亮就会触发（sql、js、python、bash、json、go 等皆然），只有未被高亮的块（无语言标注或语言不被识别）才侥幸正常。现在改为递归遍历子节点树拼接文本，可完整还原带换行的原始源码。同一处还修掉了语言标签显示错误：标签原样输出了整个 class 串（显示成 `HLJS LANGUAGE-SQL`），现改为从 class 中提取语言名，正确显示 `SQL`。

## v1.3.5 (2026-09-24)

### Added

- **codebuddy 支持发送图片（视觉能力打通）**。此前在 Web 端上传图片后再问「查看图片」，模型会回答找不到图片——图片实际上被适配器静默丢弃了，只有文件附件会落盘并在 prompt 里附上路径，图片则只写了一条 `images not supported, ignoring` 的日志。现在 codebuddy 走与 Claude Code 相同的多模态通道：把图片以 base64 塞进 `message.content` 内容块数组，并把该轮提问作为文本块一同发送。图片与当轮提示词必须组装在**同一条** stdin 帧内——CLI 会把同时灌入的多条用户帧当作各自独立的新会话，拆帧发送会导致追问时落在没有图片历史的会话里，模型同样答找不到图片。content 字段无图片时仍为纯字符串，文本链路行为完全不变；per-turn spawn 模式（走命令行参数、无内容块通道）会在 prompt 里明确告知「有 N 张图但本通道无法传递」，不再静默丢弃。

### Fixed

- **Web 端首次打开会话窗口不再停在半路，一定滚到最新消息**。此前打开会话（含刷新页面、从列表切进某个会话）时视图停在历史中段，要手动下滑或发一条消息才会到底。三个原因叠在一起：
  - **自动滚动效果压根没触发**。它靠 `消息数:末条长度:是否流式` 这个签名变化来驱动，而 `seedHistory` 合并历史时刻意保持 slice 引用不变（历史位置相同时不产生新对象），签名不变 → 效果不执行。
  - **滚动跑在内容撑高之前**。即使触发了，`requestAnimationFrame` 回调执行时 DOM 里还只是首屏高度：历史图片先用 120×80 的占位方块渲染，等进入视口后才去拉取并换成最大 220px 的真图；代码高亮、字体加载同理。结果滚到的是「占位高度版本的底部」。
  - **贴底状态被布局抖动误关**。内容异步撑高会触发 scroll 事件，此时 `scrollTop` 还在原位而 `scrollHeight` 已变大，距离判定直接超过阈值，被当成「用户主动上滑」把贴底关掉，之后所有自动滚动全部失效。

  现在改成三层防护：消息列表变化时滚动（原有逻辑）、用 `ResizeObserver` 观察内容高度变化时补滚（覆盖图片/高亮/字体的异步撑高）、并用程序滚动标志位把自己触发的 scroll 事件与用户主动滚动区分开。切换到新会话时会无条件重挂贴底状态——这是用户显式的「给我看这个会话」操作。用户主动上滑读历史仍然不会被拽回底部；点「跳到最新」或发送消息后恢复跟随。

## v1.3.3 (2026-09-21)

### Fixed

- **后台任务完成后不再「有去无回」**。让 Agent 把命令放到后台执行（CodeBuddy / Claude Code 的 `run_in_background`，以及后台子 Agent、Workflow）时，此前任务跑完后用户永远收不到任何回音——Agent 只回了「已启动」，然后就没了下文。根因是 **CLI 事件协议里的整个 `system/task_*` 事件族被适配器丢弃**：CodeBuddy 适配器只认 `system` 下的 `init` 子类型，其余落进默认分支打条 debug 日志扔掉；Claude Code 适配器更隐蔽，它只读 `system` 里的 `session_id`，压根不看 `subtype`。而这条通知链路其实早就就绪（引擎的 unsolicited reader 正是为「轮次之外的事件」准备的），断点全在上游适配器。现在两个适配器都会解析 `task_started` / `task_progress` / `task_updated` / `task_notification`，并在任务到达终态时把完成消息推送到平台。

  实现细节上处理了三个容易踩的坑：
  - **`task_notification` 不是唯一的终态信号**。CLI 明确说明终态有时**只**发 `task_updated` 不发 `task_notification`，因此二者都按终态处理，并按 `task_id` 去重，避免同一次完成推两条消息。
  - **终态不能在前台轮次进行中就推送**。引擎把 `EventResult` 当作「本轮结束」的信号（要收尾卡片、写历史、触发自动标题），若一个很快的任务在轮次内完成就推送，会把用户正在等的回复截断。判断依据是「启动该任务的那个轮次是否已结束」，而不是「当前是否有轮次在跑」——因此一个跑了很久、期间用户又问了几轮的任务，完成时依然会如实汇报，不会被后来的轮次压住。
  - **CLI 下发的任务描述属不可信文本**，可能含用户输入或子任务输出片段，且可能带换行。发往平台前统一折叠空白并截断，避免伪造消息行或撑爆平台长度限制。

  顺带修正了 Claude Code 适配器的一个小毛病：它此前会给**每个**带 `session_id` 的 `system` 事件发一条空的文本事件，而任务事件族恰好都带 `session_id`，于是会在回复里混入空片段。

## v1.3.2 (2026-09-21)

### Fixed

- **Web 端「发送/中断」按钮不再与真实运行状态脱节**。此前按钮只看浏览器当前的 WebSocket 流事件，不看后端状态，导致两个方向的错误：
  - **刷新页面后按钮变成「发送」，但会话其实还在跑**（`interruptible = false` 时最容易察觉）。刷新会清空浏览器侧的全部流事件，而历史记录里没有正在进行中的那一轮，于是前端认定空闲，用户可以在 Agent 正回答时又发一条。
  - **本轮结束后按钮仍是红色**（`interruptible = true` 时），因为结束事件丢失或本轮被打断，本地状态停在"运行中"。
  现在以管理接口的 `session.running` 为准，与本地流状态取**或**：刷新、丢帧、后台轮次结束、别的客户端停止轮次，都能被纠正。两条数据流各自维护互不覆盖——WebSocket 负责即时响应（<100ms），REST 轮询负责兜底。
- **区分「可打断」与「会排队」两种忙碌状态**。会话列表与详情接口新增 `interruptible` 字段（Agent 配置层面的能力，不是当前会话状态），Web 端据此显示不同的按钮提示：`interruptible = true` 时提示新消息会打断本轮，`false` 时提示新消息将排队。两种情况下按钮都可点击——排队的是新消息，`/stop` 永远应该能取消一个长任务。

### Changed

- **`interruptible` 配置现在会传递给多工作区（multi-workspace）模式的子 Agent**。此前该选项只作用于项目级 Agent，按工作区惰性创建的 Agent 会静默退回"排队"行为。
- **断线时的状态清理不再清除后端忙碌标志**。连接断开后本地流状态确实应当结算（终帧不可能再到达），但后端标志来自 REST，清掉它会让一个仍在运行的会话显示成空闲。
...
- **Web 端文件分享：生成免登录的只读链接**。文件抽屉（项目文件浏览器）底部新增「分享」按钮，对当前文件生成一条可直接访问的链接，**拿到链接的人无需登录、无需 token 即可查看/下载该文件**。链接**永久有效**，可随时点「取消分享」立即失效。
  - **公开范围被严格限定为「读取这一个文件」**：接口是 `GET /api/v1/share/<token>`，只读、只针对生成时的那一个文件。持有链接者**不能**列举目录、不能读取同目录的其他文件、不能调用任何其他 API —— 创建/列出/撤销分享这些管理操作**仍需要管理 token**，匿名调用一律 401。
  - **链接只绑单个文件，不是目录**：分享时若目标是目录会被拒绝；即使记录被篡改为指向目录，访问也返回 404 而不会泄露目录列表。文件被删除后链接返回 404。
  - **路径穿越防护与原有文件接口共用同一份实现**：为此把 `handleFile` 的工作目录解析与越权校验抽成 `resolveProjectFile`，两处共用，避免出现"第二份略有差异的校验"这种典型的越权漏洞来源。测试用真实的 `../` 记录验证防护确实生效（并且验证过：移除该校验时测试会失败）。
  - **分享记录持久化在 `<data_dir>/shares/shares.json`**（原子写），重启后链接依然有效、已撤销的保持失效；文件损坏时容错加载而非阻塞启动。
  - **token 为 128 位随机值**（`crypto/rand`），未知 token 与已撤销 token 返回相同响应，不泄露"曾经存在过"。
  - 界面在生成后立即复制链接到剪贴板，并以醒目提示说明"任何人拿到此链接都可直接读取该文件，无需登录"。已补 5 语言文案。
  - **21 个 Go 测试 + 4 个前端测试**覆盖：匿名可读、撤销失效、不影响其他分享、`?download=1` 强制下载、目录不可分享、不泄露目录列表、路径穿越、鉴权边界（管理操作无 token 必须 401）、不被 SPA fallback 吞掉，以及一条走真实 handler 链的端到端流程。
...
- **Web 附件上传支持 HTML 及完整文本后缀族**：消息输入框的「上传图片/文件」现可选中 `.html` / `.htm`，以及 `xml`、`svg`、`toml`、`ini`、`conf`、`cfg`、`env`、`gitignore`、`tsx`、`rs`、`rb`、`php`、`cpp`、`hpp` 等此前只能被预览、无法被上传的格式。

### Changed

- **附件接收列表改为从可预览列表派生，消除两份列表的漂移**：此前选文件器的 `accept` 是手写常量，而可内联预览的后缀清单是另一份 `TEXT_EXTS`，两者独立维护。结果是 `.html`/`.htm` 早已支持在聊天内安全预览（`isHtmlFile()`，带单测），却因为不在 `accept` 里而**根本选不中**——用户看得到预览能力却用不上。现新增 `attachmentAccept()` 从 `TEXT_EXTS` 派生 `accept`，二者不可能再分叉；「能预览却不能上传」或反之在测试里被双向断言拦截。
  - `.tar.gz` 保留为独立条目：浏览器按最后一个扩展名匹配，若被改成由 `.gz` 拼接会静默失效，故加专门用例盯住。

## v1.1.48 (2026-09-18)

### Added

- **Web 聊天消息新增时间戳展示**：每条消息气泡**上方**显示灰色小字时间（`text-[10px]`，等宽数字防抖动），悬停显示完整本地时间。用户消息是发送时刻；助手消息是**首个可见响应**时刻——第一段流式回复文本或第一个进度卡片到达时打点，本轮内冻结不再变化。展示按本地日历天分级：今天 `14:03`、昨天 `昨天 14:03`、今年更早 `9月12日 14:03`、跨年带年份。已补 5 语言文案（`chat.time.yesterday`）。
  - **修复了「实时消息无时间戳、刷新后才出现」的不一致**：历史条目本就由后端 `AddHistory` 打点并经 `history[].timestamp` 下发，前端 `ChatMsg.timestamp` 字段一直在但从未渲染，实时消息则完全没有该字段——导致刷新前无时间、刷新后有。现实时帧在落地时统一打点，刷新前后一致。
  - **相对判断按本地日历天比较**（年/月/日三元组），不是按时间差：23:59 的消息在 00:01 正确显示为「昨天」，夏令时切换也不会落错桶。已有专门单测覆盖（跨午夜边界、DST 春令切换、UTC 瞬时转本地）。
  - **时间戳在 `MessageRow` 内部计算**而非父组件预计算，避免给 `msg` 对象挂派生字段破坏 `React.memo` 的引用稳定性（长会话下会让整条消息列表在每次重渲时失效）。助手时间戳的打点遵循「只 backfill、绝不覆盖」：后续流式增量与最终 `reply` 都不会移动已有时间。
...
- **`[instant_reply]` 即时确认回复默认开启**：收到用户消息后、Agent 开始处理前立即回一条「⏳ 处理中...」，消除 CLI 启动 + 模型首 token 期间的长静默（此前默认关闭，WeCom 等平台在首帧流式输出前完全没有反馈，实测静默可达十几秒）。`content` 留空走 i18n 默认文案；显式 `enabled = false` 可退出。Web 端自动跳过（自带「正在输入」指示，常驻确认气泡是噪音）；已配置流式卡片的平台（钉钉 AI Card）维持原有自动跳过逻辑。
